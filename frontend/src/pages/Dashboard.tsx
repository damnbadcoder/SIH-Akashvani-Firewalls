...
export default function Dashboard() {
  ...
  const [sourceTab, setSourceTab] = useState<"files" | "links">("files");
  ...
  // Stage 3 clean deliverable markdown without citations
  const stage3AnnotatedContent = useMemo(() => {
    if (!active?.content) return "";
    return stripCitations(active.content);
  }, [active?.content]);
  ...
  async function finalizeGeneration() {
    ...
    try {
      ...
      for (const id of selected) {
        ...
        const content = stripCitations(res.content);
  ...
  async function retry() {
    ...
    setRetrying(true);
    try {
      const rawContent = await regenerateDeliverable(
        activeId,
        gen.sourceText,
        paramsFor(activeId),
  ...
  function acceptDraft() {
    ...
    const cleanDraft = stripCitations(draft);
    const isCurrentlyEnglish = (activeId ? paramsFor(activeId).language : "English") === "English";
    if (isCurrentlyEnglish) {
      setOriginalEnglishByOutput((prev) => ({ ...prev, [activeId]: cleanDraft }));
    }
    setGen((prevGen) => {
      if (!prevGen) return prevGen;
      return {
        ...
        deliverables: prevGen.deliverables.map((d) =>
          d.outputType === activeId
            ? {
                ...d,
                content: cleanDraft,
                originalEnglish: isCurrentlyEnglish ? cleanDraft : d.originalEnglish,
              }
            : d
        ),
      };
  ...
  async function changeDeliverableLanguage(newLang: string) {
    ...
    try {
      const rawTranslated = await translateDeliverable(
        active.content,
        newLang,
        activeId,
        sessionId || undefined,
      ...
      const translated = stripCitations(rawTranslated);
        ...
        {isReviewMode ? (
          ...
          {gen && active && !editing && (
            <div className="card retry">
              ...
              <textarea
                placeholder="Describe what to change — e.g. “shorter, drop the jargon, add a call to action”"
                value={refinement}
          ...
          {!gen && (
            <>
              ...
              <div className="card" style={{ marginBottom: "16px" }}>
                <textarea
                  className="source-text"
                  placeholder="Paste an article, report, advisory, incident note, or a free-form prompt…"
                  value={sourceText}
              ...
              {active && (
                <div className="card deliverable">
                  ...
                  {editing ? (
                    ...
                    <div className="preview-rendered-pane interactive-pane deliverable-rendered-pane">
